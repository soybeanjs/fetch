...
OpenAPI typed clients are imported from the separate `@soybeanjs/fetch/openapi` subpath for on-demand loading and smaller bundles:
...
### 18. Response Schema Validation

Validate response data at runtime via the `schema` config option. Compatible with the [Standard Schema](https://github.com/standard-schema/standard-schema) spec (Zod v4+, Valibot, ArkType, ...) and plain validator functions.

```typescript
import { z } from 'zod'; // Zod v4+ implements Standard Schema

// Using a Standard Schema (Zod / Valibot / ArkType / ...)
...
// user is runtime-validated and type-safe

// Using a plain validator function (lightweight escape hatch)
...
On validation failure a `FetchError` with `code: 'ERR_SCHEMA'` is thrown — the message includes all issue paths and descriptions. Handle it via `onError`:
...
Create a type-safe client based on `paths` types generated by openapi-typescript. Imported from the `@soybeanjs/fetch/openapi` subpath.
